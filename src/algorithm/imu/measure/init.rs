...
pub struct ImuInit<T> {
    ...
    pub linear_acc_mean: Vector3<T>,
    pub angular_acc_bias: Vector3<T>,
    ...
    T: ComplexField<RealField = T>,
{
    ...
    pub fn from_stamped_gravity(timestamp: T, gravity: Vector3<T>) -> Self {
        Self {
            ...
            linear_acc_mean: gravity,
            angular_acc_bias: Vector3::zeros(),
        ...
        I: IntoIterator<Item = StampedImu<T>>,
    {
        ...
        let StampedImu {
            ...
            *mean_acc.acc = mean_acc
                .acc
                .lerp(current_acc.acc.deref(), n.clone().recip());

            *mean_acc.gyro.deref_mut() = mean_acc.gyro.lerp(current_acc.gyro.deref(), n.recip());

            mean_acc
        });

        Some(ImuInit {
            linear_acc_norm: acc_mean.acc.norm(),
            linear_acc_mean: acc_mean.acc,
            angular_acc_bias: acc_mean.gyro.map_state_marker(),
            timestamp_init: timestamp,
        })
    }
}
