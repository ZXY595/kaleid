...
impl ToTokens for Input {
    fn to_tokens(&self, tokens: &mut TokenStream2) {
        ...
        let states_offset_impl = types
            .map(|ty| (Some(offset_ty), ty))
            .states_offset_impl(super_ty, generics);
...
pub(crate) trait IntoStateOffsetIter {
    fn states_offset_impl(
        self,
        ident_super: &Ident,
        generics: &Generics,
...
impl<'a, I> IntoStateOffsetIter for I
where
    ...
    fn states_offset_impl(
        ...
        self.scan(None, move |previous_ty, (super_ty, ty)| {
            ...
            let offset = previous_ty
                .map(|previous_ty| {
                    quote! {
                        kaleid::state::StateEnd<#previous_ty, #ident_super #ty_generics>
                    }
                })
                .or(super_ty.map(|super_ty| {
                    quote! {
                        kaleid::state::StateBegin<#super_ty #ty_generics, #ident_super #ty_generics>
                    }
                }))
                .unwrap_or(quote! { nalgebra::U0 });
            Some(quote! {
                impl #impl_generics kaleid::StateOffset<#ident_super #ty_generics> for #ty
                #where_clause
                {
                    type Offset = #offset;
                }
